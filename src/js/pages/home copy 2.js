...
const scrollGsap = async () => {
    ...
    function initStackedPanels() {
        ...
        // Set initial positions: all panels except the first one are pushed down 100%
        gsap.set(panels.slice(1), { yPercent: 100 });

        // Calculate total scroll distance needed:
        // Every panel sliding up takes 1 unit (100%). We have panels.length - 1 panels sliding.
        // Plus an extra 100% (1 unit) for the "pinned video" delay on Panel 3.
        const scrollDistance = (panels.length - 1) * 100 + 100;

        // Create a timeline linked to the scroll position of the wrapper
        ...
        // Animate each panel (except the first) up from 100% to 0% sequentially
        panels.forEach((panel, i) => {
            if (i === 0) return; // Skip the first panel as it's already in place

            // Move the panel up
            tl.to(panel, {
                yPercent: 0,
                ease: "none",
                duration: 1, // Standard duration unit for sliding up
            });

            // Special behavior for Panel 3: Simulate "Pinning" the full-screen video
            if (panel.classList.contains("panel-3")) {
                // We add an empty animation to the timeline with a duration of 3.
                // Since the wrapper is pinned, this empty animation effectively
                // keeps Panel 3 completely static for the duration of 3 "scroll units".
                tl.to({}, { duration: 1 });
            }
        });
    }
