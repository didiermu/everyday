...
const smoothLenis = async (lenis) => {
    ...
    const paneles = () => {
        ...
        panels.forEach((panel, i) => {
            ...
            if (i === 0) {
                // Primer panel (video)
                startValue = "top top";
                endValue = () => `+=${window.innerHeight * 2}`; // 200vh
            } else {
                // Demás paneles
                ((startValue = "bottom bottom"),
                    // startValue = () => `top+=${window.innerHeight} top`; // retrasa 100vh
                    (endValue = "bottom top"));
            }
    ...
    const scrollViz = async () => {
        ...
        if (scrollVizTrigger) {
            scrollVizTrigger.kill();
            scrollVizTrigger = null;
        }
        ...
        scrollVizTrigger = ScrollTrigger.create({
